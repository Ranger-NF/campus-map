...
func _unhandled_input(event):
	if event.is_action_pressed("zoom_in"):
		set_zoom_level(zoom_level + GlobalVariables.zoom_factor)
		...
		if event.factor > 1:
			set_zoom_level(zoom_level + (GlobalVariables.zoom_factor * 0.5))
		elif event.factor < 1:
			set_zoom_level(zoom_level - (GlobalVariables.zoom_factor * 0.5))
...
func set_zoom_level(level: float, mouse_world_position = self.get_global_mouse_position()):
	...
	zoom_level = clampf(level, GlobalVariables.min_zoom, GlobalVariables.max_zoom)
